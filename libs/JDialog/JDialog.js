...
(function($) {
	...
	window.JDialog = {
		...
		alert : function(options) {
			...
			var o = this.open({
				title : options.title,
				content : '<div style="padding-top:13px; font-size:16px; vertical-align: middle;">'+options.content+'</div>',
				icon : options.icon,
				width : options.width,
				closeBtn : true,
				button : {
		...
		confirm : function(options) {
			options.content = '<div style="padding-top:13px; font-size:14px;">'+options.content+'</div>';
			if (!options.width) {
				options.width = 300;
			}
